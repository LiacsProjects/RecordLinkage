...
class RecordLinker():
    ...
    def get_period(self, mode, age=0):
        start = None
        end = None
        if mode == 1:
    ...
    def find_links(self, mode):
        ...
        for reference in df_references.itertuples():
            ...
            for potential_link in df_potential_links_filtered.itertuples():
                ...
                if [mode, potential_link.man_uuid, reference.man_uuid, "m"] in self.links_persons:
                    continue
                ...
                if distance <= MAX_LEVENSTHEIN:
                    ...
                    self.links_persons.append([mode, reference.man_uuid, potential_link.man_uuid, "m"])
                    self.links_persons.append([mode, reference.woman_uuid, potential_link.woman_uuid, "v"])
